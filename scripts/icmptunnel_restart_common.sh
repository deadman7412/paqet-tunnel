#!/usr/bin/env bash
# Shared restart protection logic for ICMP Tunnel
...
get_state_file() {
  local role="$1"
  echo "/var/tmp/icmptunnel_restart_state_${role}.txt"
}

get_lock_file() {
  local role="$1"
  echo "/var/tmp/icmptunnel_restart_${role}.lock"
}
...
safe_restart_service() {
  local role="$1"
  local source="$2"  # "health" or "cron"
  local service_name="icmptunnel-${role}"
  local state_file
  state_file="$(get_state_file "${role}")"
  local lock_file
