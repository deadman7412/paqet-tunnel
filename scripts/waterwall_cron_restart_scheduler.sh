...
CRON_FILE="/etc/cron.d/waterwall-restart-${SERVICE_NAME}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESTART_SCRIPT="${SCRIPT_DIR}/waterwall_cron_restart.sh"
LOG_FILE="/var/log/waterwall-cron-${ROLE}.log"
...
while true; do
  ...
  echo
  echo "=============================================="
  echo "WaterWall Scheduled Restart: ${SERVICE_NAME}"
  echo "=============================================="
  ...
  cat > "${CRON_FILE}" <<CRON
# WaterWall Scheduled Restart for ${SERVICE_NAME}
