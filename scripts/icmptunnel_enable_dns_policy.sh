...
if ! id -u "${ICMPTUNNEL_USER}" >/dev/null 2>&1; then
  useradd --system --no-create-home --shell /usr/sbin/nologin "${ICMPTUNNEL_USER}"
fi
...
iptables -t nat -A OUTPUT -m owner --uid-owner "${ICMPTUNNEL_UID}" -p udp --dport 53 -m comment --comment icmptunnel-dns-policy -j REDIRECT --to-ports "${DNS_PORT}"
iptables -t nat -A OUTPUT -m owner --uid-owner "${ICMPTUNNEL_UID}" -p tcp --dport 53 -m comment --comment icmptunnel-dns-policy -j REDIRECT --to-ports "${DNS_PORT}"
...
set_state "icmptunnel_dns_enabled" "1"

echo "DNS policy binding enabled for icmptunnel traffic."
