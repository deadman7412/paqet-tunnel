...
ensure_server_iptables_rules() {
  ...
  if iptables -t raw -A PREROUTING -p tcp --dport "${port}" -m comment --comment paqet-notrack-in -j NOTRACK 2>&1; then
  ...
  if iptables -t raw -A OUTPUT -p tcp --sport "${port}" -m comment --comment paqet-notrack-out -j NOTRACK 2>&1; then
  ...
  if iptables -t mangle -A OUTPUT -p tcp --sport "${port}" --tcp-flags RST RST -m comment --comment paqet-rst-drop -j DROP 2>&1; then
  ...
  iptables -t raw -L PREROUTING -n -v 2>/dev/null | grep -i "paqet-notrack" | sed 's/^/  /' || echo "  (none found)"
  iptables -t raw -L OUTPUT -n -v 2>/dev/null | grep -i "paqet-notrack" | sed 's/^/  /' || echo "  (none found)"
  echo "[DEBUG] mangle table RST drop rules:"
  iptables -t mangle -L OUTPUT -n -v 2>/dev/null | grep -i "paqet-rst" | sed 's/^/  /' || echo "  (none found)"
