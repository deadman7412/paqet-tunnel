...
CRON_FILE="/etc/cron.d/waterwall-health-${SERVICE_NAME}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HEALTH_CHECK_SCRIPT="${SCRIPT_DIR}/waterwall_health_check.sh"
LOG_FILE="/var/log/waterwall-health-${ROLE}.log"
...
while true; do
  ...
  echo
  echo "========================================="
  echo "WaterWall Health Check: ${SERVICE_NAME}"
  echo "========================================="
  ...
  cat > "${CRON_FILE}" <<CRON
# WaterWall Health Check for ${SERVICE_NAME}
